...
//        思路参考 isalicema/m5-faces-core-s3（MIT），实现为本仓库独立编写，见文件末尾致谢。
//
...
//
// ── 致谢 / Credits ──────────────────────────────────────────────────────
// 用 Faces 套件当「Mac 的音乐遥控器」这个整体思路（设备只做控制与显示、声音永远
// 由电脑出；控制走 HID 媒体键，正在播放的信息回传到设备屏幕上）参考了
//   https://github.com/isalicema/m5-faces-core-s3   （MIT License）
// 本文件的实现是独立编写的：NimBLE 自定义 GATT 回传歌名/歌手/专辑/进度/封面，
// 封面走 160×160 JPEG 分包协议（见 config.h）。未复制其代码。
